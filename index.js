...
function main(){
	...
	for(let xVal = 0; xVal < 8; xVal++){
		...
		for(let yVal = 0; yVal < 8; yVal++){
			...
			//sets red and black peices depending on the y value
			if(yVal < 3 && tiles[xVal][yVal].deadSpace){ 
				tiles[xVal][yVal].chip = true;  	 //if a chip in on the tile
				tiles[xVal][yVal].chipColor = true;  //what coloe the chip is (red is true)
...
function nextTurn(){
	...
	//resets the hand to a invaild position and changes if its highlighting to false
	hand.resetHand();
	
	//resets jumpagain to false to end double jump
	jumpAgain = false;
...
function screenClicked(event){
	...
	if(xPos > board.Hoffset && xPos < board.Hoffset + board.size){
		if(yPos > board.Voffset && yPos < board.Voffset + board.size){
			...
			//if you havent clicked on a move and if you are not locked into jumping again it will let you click on a new piece
...
function checkMoveMoves(xIn, yIn){
	...
	if(tiles[xIn][yIn].chip){
		...
		//it then makes sure the chip isnt by the edge of the board
		//this will break the code if you check in a space that does not exsist(edge case)
		...
		if(xIn+1 < 8 && yIn+dir < 8 && yIn+dir >= 0){
			...
			//checks if the space to the right of the peice is open
			if(!tiles[xIn+1][yIn+dir].chip){
		...
		if(xIn-1 >= 0 && yIn+dir < 8 && xIn-2 >= 0 && yIn+(2*dir) < 8 && yIn+dir >= 0 && yIn+(2*dir) >= 0){
			...
			if(tiles[xIn-1][yIn+dir].chipColor != tiles[xIn][yIn].chipColor && tiles[xIn-1][yIn+dir].chip){
				...
				if(!tiles[xIn-2][yIn+(2*dir)].chip){
					...
					possibleMoves.push({x: xIn-2,         //new location of chip
										y: yIn + (2*dir),
										type: "jump",
		...
		if(xIn+1 < 8 && yIn+dir < 8 && xIn+2 < 8 && yIn+(2*dir) < 8 && yIn+dir >= 0 && yIn+(2*dir) >= 0){
			...
			if(tiles[xIn+1][yIn+dir].chipColor != tiles[xIn][yIn].chipColor && tiles[xIn+1][yIn+dir].chip){
				...
				if(!tiles[xIn+2][yIn+(2*dir)].chip){
					...
					//pushs jump object into array
					possibleMoves.push({x: xIn+2,
										y: yIn + (2*dir),
										type: "jump",
										x2: xIn+1,
										y2: yIn+dir});
		...
		if(xIn-1 >= 0 && yIn-dir >= 0 && xIn-2 >= 0 && yIn-(2*dir) >= 0 && yIn-dir < 8 && yIn-(2*dir) < 8){
			...
			if(tiles[xIn-1][yIn-dir].chipColor != tiles[xIn][yIn].chipColor && tiles[xIn-1][yIn-dir].chip){
				...
				if(!tiles[xIn-2][yIn-(2*dir)].chip){
					...
					possibleMoves.push({x: xIn-2,          //new coords
										y: yIn - (2*dir),
										type: "jump",
										x2: xIn-1,         //piece that was jumped's coords
										y2: yIn-dir});
				}
			}
		}
...
function Hand(){
	...
	this.checkAllMoves = function(){
		...
		if(!jumpAgain){
			...
			//this will set the possibleMoves as a concatination of the previous possibleMoves and the return values
	...
	this.draw = function(){
		...
		if(this.highlight){
			...
			for(let movesInt = 0; movesInt < this.possibleMoves.length; movesInt++){
				...
				ctx.fillRect((this.possibleMoves[movesInt].x*this.width) + board.Hoffset, (this.possibleMoves[movesInt].y*this.height) + board.Voffset, this.width, this.height);
			}
...
function Tile(xIn, yIn){
	
	//here are the default values
	this.deadSpace = false;         //deadspace is kinda a backwards name becuase if its true that means it is a real space that chips can go on
	...
	this.draw = function(){
		...
		if(this.deadSpace){
			...
			//creates a normal rectable based on the position of the tile and the offset of the board
			ctx.fillStyle = this.color;
			...
			if(this.chip){
				...
				//it then sets the fill and stroke colors based on what clor chip is on the tile (true is red)
				if(this.chipColor){
